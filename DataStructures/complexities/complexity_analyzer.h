...
namespace ds::utils
{
    ...
    protected:
        ComplexityAnalyzer(const std::string& name, std::function<void(Structure&, size_t)> insertN);
    ...
    template <class Structure>
    ComplexityAnalyzer<Structure>::ComplexityAnalyzer(const std::string& name, std::function<void(Structure&, size_t)> insertN) :
        LeafAnalyzer(name),
        insertN_(insertN) {}
    ...
    void ComplexityAnalyzer<Structure>::analyze(Structure structurePrototype)
    {
        ...
        for (int r = 0; r < this->getReplicationCount(); ++r)
        {
            ...
            {
                size_t dataSize = this->getStepSize() * (step + 1);
                insertN_(structure, dataSize - structure.size());
                this->beforeOperation(structure);
                auto startTime = std::chrono::high_resolution_clock::now();
                this->executeOperation(structure);
                auto endTime = std::chrono::high_resolution_clock::now();
                this->afterOperation(structure);
                auto executionTime = std::chrono::duration_cast<duration_t>(endTime - startTime);
                samples[dataSize].push_back(executionTime);
            }
        }
