...
namespace ds::utils
{
    ...
    {
    public:
        ListsAnalyzer() : CompositeAnalyzer("Lists")
        {
    ...
    ListAnalyzer<List>::ListAnalyzer(const std::string& name) :
        ComplexityAnalyzer<List>(name, [this](List& list, size_t size)
            {
                this->insertNElements(list, size);
            }),
    ...
    template <class List>
    ListInsertAnalyzer<List>::ListInsertAnalyzer(const std::string& name) : ListAnalyzer<List>(name) {}
    ...
    template <class List>
    ListRemoveAnalyzer<List>::ListRemoveAnalyzer(const std::string& name) : ListAnalyzer<List>(name) {}
