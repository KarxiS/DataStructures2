...
namespace ds::adt {
    ...
    public:
       ...
       /* zmena tychto 2 riadkov, su vytiahnute z protected do publicu aby som mohol prehladavat pri ukladani udajov do tabulky s log zlozitostou */
        using BlockType = typename amt::IS<TabItem<K, T>>::BlockType;
    ...
    T HashTable<K, T>::remove(K key)
    {
        ...
        SynonymTable* synonyms = primaryRegion_->access(index)->data_;
        if (synonyms == nullptr) {
            this->error("Hash tab hasn't this key");
        }
    ...
    auto HashTable<K, T>::HashTableIterator::operator++() -> HashTableIterator&
    {
        ...
        SynonymTableIterator temp = *this->synonymIterator_;
